...
class DeepSeekProxyHandler(BaseHTTPRequestHandler):
    ...
    _thinking_indices: set[int]

    def setup(self) -> None:
        super().setup()
        self._thinking_indices = set()
    ...
    def _print_streaming_chunk(self, chunk: dict[str, Any]) -> None:
        GREEN = "\033[32m"
        RESET = "\033[0m"
        ...
        for choice in choices:
            ...
            # Track thinking state via markers
            remaining = content
            output_parts: list[str] = []
            while remaining:
                if remaining.startswith(THINKING_BLOCK_START):
                    self._thinking_indices.add(index)
                    output_parts.append(GREEN + THINKING_BLOCK_START)
                    remaining = remaining[len(THINKING_BLOCK_START) :]
                elif THINKING_BLOCK_END in remaining:
                    pos = remaining.index(THINKING_BLOCK_END)
                    thinking_text = remaining[: pos + len(THINKING_BLOCK_END)]
                    output_parts.append(thinking_text + RESET)
                    remaining = remaining[pos + len(THINKING_BLOCK_END) :]
                    self._thinking_indices.discard(index)
                else:
                    break

            # Print remaining content with appropriate color
            if remaining:
                if index in self._thinking_indices:
                    output_parts.append(GREEN + remaining + RESET)
                else:
